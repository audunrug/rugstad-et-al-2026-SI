---
title: "Supporting Information 2: Worked examples"
format: pdf
editor: visual
...
One important note is that all code here is run using `gllvm` version 2.0.10. If users encounter errors that prevents the code from running, it will likely be due to version mismatch. Note also that the `gllvm` model fitting function will sometimes give warnings which *contain* errors from parts of the model fitting. This might happen e.g. when one of several model fitting iterations fail to converge to a stable solution, and does not need to mean that the model fitting as a whole (across multiple iterations) was unsuccessful.
...
Below is the code for the fitting of the unconstrained and concurrent model-based ordinations. All models ran without giving errors.
...
for (n_lv in 1:6) { # run up to 6 LVs
  print(paste("Fitting model with:", n_lv, "latent variables"))
  ...
  n.init = 5, # number of starting iterations (to ensure convergence)
...
@tbl-1-aic1 shows the Aikake Information Criteria (AIC) for the resulting unconstrained latent variable models, while @fig-1-diag1 and @fig-1-diag2 show the diagnostic plots, obtained using the `plot()` function on the `gllvm` model object. We see that there is an AIC minimum for the model with 5 latent variables.
...
As seen in the top row in both @fig-1-diag1 and @fig-1-diag2, there is a slight over-representation of negative residuals in all models for high values of the linear predictor. However, we assume it is not large enough to significantly impact the predictions of the model. Otherwise, all the other residual diagnostics plots seem to indicate that the model assumptions hold for our data, for all models.

```{r}
#| echo: true
#| label: fig-1-diag1
#| fig-cap: "Diagnostic plots of normal randomized quantile residuals for the unconstrained models in worked example 1. Left column = 1 latent variable, middle = 2 and right = 3."
#| fig-width: 7
#| fig-height: 9
...
#| echo: true
#| label: fig-1-diag2
#| fig-cap: "Diagnostic plots of normal randomized quantile residuals for the unconstrained models in worked example 1. Left column = 4 latent variable, middle = 5 and right = 6."
#| fig-width: 7
#| fig-height: 9
...
We see that some of the iterations produce warnings from the `L-BFGS-B` optimiser. However, this does not necessarily mean that there is any issues with the final fitted model.

Still, looking at the sign of the diagonal of the covariance matrix for the parameter estimators, we do see that several of the models give negative values, which indicates poor model fit, even after running for several iterations. Based on this, we decide to stick with the model with three latent variables for further analysis, as it does not have any negative values, even though the 5 latent variable model had lower AIC.
...
```{r}
goodnessOfFit(object=we_1_uc_mod)
```

### Concurrent latent variable model

Below is the model summary of the concurrent ordination model with two concurrent latent variables. we see that the parameter estimates for the effects of soil moisture and soil nitrogen content on latent variable 1 are the only canonical coefficients significant at a 95% confidence level.
...
```{r}
#| label: fig-1-cor3
#| fig-cap: "Residual between-species correlation from the concurrent ordination (after accounting for the canonical coefficients."
#| fig-width: 7
#| fig-height: 5
...
Looking at the corrogram for the continuous predictors [@fig-2-cor], we see no significant co-linearity, except for the relationship between canopy cover and time since restoration, which is likely due to the fact that all sites in the reference category are coded with a high "years since restoration" value of 40. And since we are not planning on using this value in our model, see below, it will not be a problem for us.
...
Indeed, looking only at the correlation for the non-reference sites reveals that both the strong correlations between canopy cover and distance to vegetation with year disappears.

```{r}
#| label: fig-2-cor2
#| fig-cap: "Correlation plot for the observed environmental predictors in worked example 1, for the subset of non-reference sites."
#| fig-width: 7
#| fig-height: 6
env_sub |> 
  filter(method != "ref") |> 
...
We try to fit a concurrent model using the ordered beta distribution. To do this, we have to convert our species data into fractions on the 0-1 interval:
...
We then attempt to fit the model with 1 to 3 latent variables. Because they took some time to fit with each iteration, we kept `n.init = 2` for all runs. Although we acknowledge that, given more patience by the authors, the model fit could probably be improved through more iterations.
...
for (n_lv in 1:3) {  # run up to 3 LVs
  we_2_concurrent[[n_lv]] <- gllvm(
    ...
    studyDesign = env, # study design matrix (for row.eff)
...
#| output: true
#| label: fig-2-diag1
#| fig-cap: "Diagnostic plots of normal randomized quantile residuals for the concurrent models in worked example 2. Left column = 1 latent variable, middle = 2 and right = 3."
#| fig-width: 7
#| fig-height: 9
layout(mat = matrix(1:15, nrow = 5, ncol = 3))
...
We therefore decide to proceed with the 1-latent variable model for the visualisations and analysis.
...
We also see that many of the species loadings estimated by the 2-lv model are both severely "blown up" and almost perfectly positively correlated, supporting the notion that the model is poorly converged, and that there is not separate second community gradient that is represented with one latent variable
...
```{r}
pred_scores_0 <- predict.gllvm(object = we_2_cn_mod, 
                               type = "response")
...
```{r}
#| label: fig-2-vp
#| fig-cap: "Variance partitioning of the latent variable model in worked example 2"
#| fig-width: 8
#| fig-height: 7
vp_we_2_cn_mod <- varPartitioning.gllvm(we_2_cn_mod,
                                        group=c(2,2,2,
                                                ...
                                                4,4,4,4,4,
                                                5),
                                        groupnames=c("Site RE (species-specific)", 
                                                     "Rest. treatment",
                                                     "Rest. treatment X time",
                                                     "Other environmental predictors",
                                                     "Plot RE"
                                                     ))

# order by proportion explained by treatment
...
Another visualisation is a boxplot of the distribution of the random effects, both sample and species specific. We see for example that the sites in region 3 had an overall greater abundances of species.
